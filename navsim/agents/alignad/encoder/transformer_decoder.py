...
class MyTransformeDecoder(nn.Module):
    ...
    def forward(self,query,keyval,pos_embed=None,tgt_mask=None):
        # Ensure all operations happen on the same device as keyval
        device = keyval.device
        
        if query is not None:
            query_feature=self.in_proj(query)
        else:
            # Ensure embedding is on the same device as keyval
            query_feature=self.init_feature.weight[None].repeat(keyval.shape[0],1, 1).to(device)

        if self.add_embeding:
            query_feature=query_feature[:,None]+ self.add_embed.weight[None,:,None].to(device)

        query_feature=query_feature.reshape(query_feature.shape[0], -1, query_feature.shape[-1])

        if pos_embed is not None:
            query_feature=query_feature+pos_embed.to(device)
