...
RandomScaleImageMultiViewImage = RandomScaleImageMultiViewImage(scales=[0.2])
...
def _get_bev_feature(agent_input, training: bool=False):
    ...
    features = {"camera_feature": camera_feature,
                "img_shape": torch.FloatTensor(np.stack(image_result["img_shape"])),
                "lidar2img": torch.FloatTensor(np.stack(image_result["lidar2img"]))
                }
