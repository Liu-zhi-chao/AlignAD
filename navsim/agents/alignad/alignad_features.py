...
from .encoder.bev_feature_build import _get_bev_feature


class AlignADFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        lidar_pc = agent_input.lidars[-1].lidar_pc[[0, 1, 2]].T
...
class AlignADTargetBuilder(AbstractTargetBuilder):
    def __init__(self, config: AlignADConfig):
        self._config = config
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        bev_occ_xy, bev_occ_label = self._compute_bev_semantic_map(
        ...
        return {
            ...
            "bev_occ_xy": bev_occ_xy,
            "bev_occ_label": bev_occ_label,
    ...
    def _compute_agent_targets(self, annotations: Annotations) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        def _xy_in_lidar(x: float, y: float, config: AlignADConfig) -> bool:
    ...
    def _compute_bev_semantic_map(
        ...
        # bev_occ_xy = np.zeros((*self._config.bev_semantic_frame, 2), dtype=np.int64)
        bev_occ_label = np.zeros(self._config.bev_semantic_frame, dtype=np.int64)
        for label, (entity_type, layers) in self._config.bev_semantic_classes.items():
            ...
            bev_occ_label[entity_mask] = label

        bev_occ_x = np.linspace(
            self._config.pc_range[0] + self._config.bev_pixel_size/2,
            self._config.pc_range[3] - self._config.bev_pixel_size/2,
            num=self._config.lidar_resolution_width,
        )
        bev_occ_y = np.linspace(
            self._config.pc_range[1] + self._config.bev_pixel_size/2,
            self._config.pc_range[4] - self._config.bev_pixel_size/2,
            num=self._config.lidar_resolution_height,
        )
        bev_occ_y, bev_occ_x = np.meshgrid(bev_occ_y, bev_occ_x)
        bev_occ_xy = np.concatenate(
            (bev_occ_x[..., None], bev_occ_y[..., None]), axis=-1
        )

        return torch.Tensor(bev_occ_xy), torch.Tensor(bev_occ_label)
    ...
    def _compute_map_polygon_mask(
        ...
        map_polygon_mask = np.rot90(map_polygon_mask)[::-1]  # Transpose here
    ...
    def _coords_to_pixel(self, coords):
        ...
        pixel_center = np.array(
            [[self._config.bev_pixel_height / 2.0, self._config.bev_pixel_width / 2.0]]
        )
