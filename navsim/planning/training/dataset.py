...
def load_feature_target_from_pickle(path: Path) -> Dict[str, torch.Tensor]:
    ...
    # Convert numpy arrays to torch tensors, casting float16 to float32 to preserve training precision
    converted_dict = {}
    for key, value in data_dict.items():
        if isinstance(value, np.ndarray):
            # Float16 numpy arrays get promoted to float32 torch tensors
            if value.dtype == np.float16:
                converted_dict[key] = torch.from_numpy(value.astype(np.float32))
            else:
                converted_dict[key] = torch.from_numpy(value)
        else:
...
class MetricDataset(torch.utils.data.Dataset):
    ...
    def _cache_scene_with_token(self, token: str) -> None:
        ...
        agent_input = scene.get_agent_input()
        ######## debug ########
        # plot_cameras_frame_with_lidar(scene, 3)
